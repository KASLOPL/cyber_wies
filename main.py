...
player_x_start_middle = None
player_y_start_middle = None
player_x_start_cyber = None
player_y_start_cyber = None
...
def show_in_wall_comunicate():
    '''
        #? Funkcja Wyświetla na ekranie komunikat o tym że gracz jest w ścianie
    '''
    comunicat_color = (0, 0, 0, 128)
    comunicate_surface = pygame.Surface((800, 800), pygame.SRCALPHA)
    comunicate_surface.fill(comunicat_color)
    comunicate_font = pygame.font.Font(None, 74)
    comunicate_text_1 = comunicate_font.render("Jesteś w ścianie,", True, (255, 255, 255))
    comunicate_text_2 = comunicate_font.render("Zostaniesz cofnięty!", True, (255, 255, 255))
    display.blit(bg, (0, 0))
    if world == 1 and where_hole == "middle":
        display.blit(hole, (hole_x, hole_y))
    if world == 2 and where_hole == "cyber":
    ...
    for block in current:
        display.blit(block[0], (block[1], block[2]))
    display.blit(comunicate_surface, (0 ,0))
    display.blit(comunicate_text_1, (200, 350))
    display.blit(comunicate_text_2, (180, 410))
    pygame.display.flip()
...
while run and not start_screen2:
    ...
    if keys[pygame.K_SPACE] and not space and not moving:
        ...
        cool_down = 100
        if world == 1:
            player_x_start_middle = player_x
            player_y_start_middle = player_y
            world = 2
            current = blocks_2
            transition_surface = pygame.Surface((800, 800), pygame.SRCALPHA)
            for alpha in range(0, 256, 8):
            ...
            player_x_start_cyber = player_x
            player_y_start_cyber = player_y
            world = 1
            current = blocks_1
            transition_surface = pygame.Surface((800, 800), pygame.SRCALPHA)
            for alpha in range(0, 256, 8):
        ...
        if is_in_block(player_x, player_y):
            ...
            show_in_wall_comunicate()

            if world == 1:
                player_x = player_x_start_middle
                player_y = player_y_start_middle
            if world == 2:
                player_x = player_x_start_cyber
                player_y = player_y_start_cyber
